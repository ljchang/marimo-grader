...
# dependencies = ["marimo", "numpy>=2", "marimo-grader-client", "mograder"]
...
def test_rewrites_client_and_pins_mograder(monkeypatch):
    monkeypatch.setenv("GRADER_CLIENT_REQUIREMENT", "marimo-grader-client @ file:///app/client")
    ...
    assert '"marimo-grader-client @ file:///app/client"' in line
    assert '"mograder==' in line
    assert '"marimo-grader-client"' not in line.replace("marimo-grader-client @", "")
...
def test_adds_missing_requirements():
    ...
    assert "marimo-grader-client" in out and "mograder" in out
