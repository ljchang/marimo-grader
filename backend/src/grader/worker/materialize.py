...
packages a MoLab/WASM session installs, including ``marimo-grader-client`` and
...
* ``marimo-grader-client`` may not be on an index yet (or the worker should use the
  exact version it ships with). ``GRADER_CLIENT_REQUIREMENT`` overrides the
  requirement string, e.g. ``marimo-grader-client @ file:///app/client`` in the
...
def _client_requirement() -> str:
    return os.environ.get("GRADER_CLIENT_REQUIREMENT") or "marimo-grader-client"
...
def rewrite_dependencies(source: str) -> str:
    """Return ``source`` with the worker's requirement strings for marimo-grader-client and mograder."""
    ...
    for item in items:
        ...
        seen.add(key)
        out.append(item)
    if "marimo-grader-client" not in seen:
